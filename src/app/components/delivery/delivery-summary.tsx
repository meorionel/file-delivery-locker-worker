...
export function DeliverySummary({ delivery, textPreview, statusText, variant = "locker" }: DeliverySummaryProps) {
	...
	const containerClass = isGuest ? "flex flex-col gap-4" : "min-w-0 w-full border-t border-[rgba(250,249,245,0.1)] pt-[18px] flex flex-col gap-4";
	...
	const gridClass = isGuest ? "grid grid-cols-2 gap-3 text-sm" : "grid grid-cols-2 gap-3 text-sm";
	...
	return (
		<div className={containerClass}>
			...
			<div className={gridClass}>
				{isGuest && <Mini label={t("guest.status")} value={statusLabel} />}
				<Mini label={t("pickup.remaining")} value={remainingLabel} />
				<Mini label={t("pickup.expires")} value={formatTime(delivery.expiresAt, locale, t("common.forever"))} />
				{isGuest && <Mini label={t("guest.size")} value={formatBytes(delivery.size)} />}
			</div>
		</div>
	);
}
