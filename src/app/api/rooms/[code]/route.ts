...
export async function GET(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
	...
	const room = await db.prepare("SELECT code, created_at FROM rooms WHERE code_hash = ?").bind(codeHash).first<{ code: string; created_at: number }>();

	if (!room) {
		return json({ error: "Room not found" }, 404);
	}

	const fileCount = await db.prepare("SELECT COUNT(*) as count FROM room_files WHERE room_code_hash = ? AND deleted_at IS NULL").bind(codeHash).first<{ count: number }>();
