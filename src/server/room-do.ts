...
export class Room {
	...
	private async ensureRoomState(code: string): Promise<void> {
		...
		const row = await this.env.DB.prepare("SELECT code_hash, code, storage_prefix, created_at, last_activity_at FROM rooms WHERE code_hash = ?")
			.bind(codeHash)
	...
	private async validateJoinToken(token: string): Promise<boolean> {
		...
		const row = await this.env.DB.prepare("SELECT room_code_hash FROM rooms_join_tokens WHERE token_hash = ? AND expires_at > ?")
			.bind(tokenHash, now)
			.first<Record<string, unknown>>();
	...
	private async destroyRoom(state: RoomStoredState): Promise<void> {
		...
		await db.prepare("DELETE FROM room_files WHERE room_code_hash = ?").bind(state.codeHash).run();

		await db.prepare("DELETE FROM rooms_join_tokens WHERE room_code_hash = ?").bind(state.codeHash).run();

		await db.prepare("DELETE FROM rooms WHERE code_hash = ?").bind(state.codeHash).run();
